...
impl ModelTheftGuard {
    pub fn new(max_distinct_queries_per_hour: usize) -> Self {
        Self {
            distillation_patterns: vec![
                Regex::new(r"(?i)generate\s+(\d+|many|all)\s+(examples|samples|rows|records)")
                    .unwrap(),
                Regex::new(
                    r"(?i)(print|reveal|show|dump)\s+(your|system)\s+(instructions|prompt|rules)",
                )
                .unwrap(),
    ...
    pub fn detect_extraction_attempt(
        &mut self,
        user_id: &str,
        query: &str,
