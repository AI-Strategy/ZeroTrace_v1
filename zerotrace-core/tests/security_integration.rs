use zerotrace_core::interceptor::universal_guard::ProductionGuard as UniversalGuard;

#[tokio::test]
async fn test_universal_guard_clean_flow() {
    let guard = UniversalGuard::new();
    let result = guard
        .evaluate_complete_risk_profile_legacy("Hello AI, what is the weather?", "user_123")
        .await;
...
async fn test_universal_guard_block_injection() {
    ...
    let result = guard
        .evaluate_complete_risk_profile_legacy(injection, "attacker_01")
        .await;
...
async fn test_universal_guard_pii_redaction() {
    ...
    let result = guard
        .evaluate_complete_risk_profile_legacy(pii_prompt, "user_pii")
        .await;
    ...
    assert!(
        scrubbed.contains("[PII:v1:EMAIL:"),
...
async fn test_universal_guard_crescendo_escalation() {
    ...
    let _ = guard
        .evaluate_complete_risk_profile_legacy("sudo exec rm -rf /", "bad_actor")
        .await;
}
