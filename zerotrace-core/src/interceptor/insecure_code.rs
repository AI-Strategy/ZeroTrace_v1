...
impl FunctionRegistry {
    ...
    pub fn invoke_function(&self, function_name: &str, params: Value) -> Result<String, SecurityError> {
        ...
        if let Some(func) = self.audited_functions.get(function_name) {
            ...
            Err(SecurityError::InsecureCodeExecutionAttempt(function_name.to_string()))
        }
    }
}
...
fn safe_calculate_interest(params: Value) -> Result<String, String> {
    let principal = params.get("principal").and_then(|v| v.as_f64()).ok_or("Missing principal")?;
    let rate = params.get("rate").and_then(|v| v.as_f64()).ok_or("Missing rate")?;
    let interest = principal * rate;
    Ok(format!("{:.2}", interest))
}

fn safe_format_date(params: Value) -> Result<String, String> {
    let date_str = params.get("date").and_then(|v| v.as_str()).ok_or("Missing date")?;
