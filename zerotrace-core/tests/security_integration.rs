...
async fn test_universal_guard_clean_flow() {
    ...
    let result = guard.evaluate_complete_risk_profile("Hello AI, what is the weather?", "user_123").await;
...
async fn test_universal_guard_block_injection() {
    ...
    let result = guard.evaluate_complete_risk_profile(injection, "attacker_01").await;
    assert!(result.is_err(), "Injection should be blocked");
    let err_msg = result.err().unwrap();
    assert!(err_msg.contains("LLM01"), "Should be blocked by LLM01 Sentinel or DBS");
}
...
async fn test_universal_guard_pii_redaction() {
    ...
    let result = guard.evaluate_complete_risk_profile(pii_prompt, "user_pii").await;
    
    assert!(result.is_ok());
    let scrubbed = result.unwrap();
    assert!(!scrubbed.contains("test@example.com"), "Email should be redacted");
    assert!(scrubbed.contains("EMAIL-UUID"), "Should contain redaction token");
}
...
async fn test_universal_guard_crescendo_escalation() {
    ...
    let _ = guard.evaluate_complete_risk_profile("sudo exec rm -rf /", "bad_actor").await;
}
