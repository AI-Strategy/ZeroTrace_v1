...
fn test_semantic_caching_zero_cost() {
    let mut guard = FinancialGuard::new(Decimal::new(1000, 2), Decimal::new(1, 0)).unwrap();
...
fn test_spend_velocity_spike() {
    ...
    assert!(matches!(
        result,
        Err(SecurityError::SpendVelocityExceeded { .. })
    ));
}
...
fn test_extreme_token_counts() {
    ...
    let metrics = guard
        .check_budget(100_000_000, Decimal::ZERO, false)
        .unwrap();
