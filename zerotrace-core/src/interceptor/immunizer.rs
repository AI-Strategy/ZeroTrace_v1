...
impl ImmunizationRule {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if !(0.0..=1.0).contains(&self.false_positive_risk) {
            return Err(ImmunizationError::InvalidConfidence(
                self.false_positive_risk,
            ));
        }

        // Validate threat type
        if self.threat_type.trim().is_empty() {
            return Err(ImmunizationError::MissingField("threat_type".to_string()));
        }
...
pub async fn generate_rule_from_research(
    ...
    let pattern = match result.recommended_rule_pattern {
        ...
        None => {
            debug!(
                "No rule pattern recommended for threat type: {}",
                threat_type
            );
            return Ok(None);
        }
    ...
    let state = determine_rule_state(result.confidence_score, result.false_positive_risk, &config);
...
fn determine_rule_state(confidence: f32, fp_risk: f32, config: &RuleGenerationConfig) -> RuleState {
    if confidence > config.active_confidence_threshold && fp_risk < config.active_fp_threshold {
        RuleState::Active
...
async fn log_rule_to_graph(rule: &ImmunizationRule) -> Result<()> {
    let message = format!(
...
async fn push_to_global_feed(rule: &ImmunizationRule, config: &DeploymentConfig) -> Result<()> {
    info!(
...
mod tests {
    ...
    fn test_rule_state_determination() {
        ...
        assert_eq!(determine_rule_state(0.75, 0.1, &config), RuleState::Review);
    }
